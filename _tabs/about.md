---
# the default layout is 'page'
icon: fas fa-info-circle
order: 4
---

![](/assets/images/avatar.png)

Hello, I'm Pylon, a young student of cybersecurity. I'm focused in web pentesting. In my free time, I dedicate myself to creating CTFs for platforms like Dockerlabs, HackMyVM, and BugBountyLabs. My certificates objetives are:

<div style="display: flex; justify-content: center; gap: 40px; margin-top: 20px;">
  <div style="text-align: center;">
    <img src="/assets/images/about/bscp.png" alt="BSCP" style="width: 100px; height: auto;">
    <div style="font-weight: bold; font-size: 1.2em; margin-top: 10px;">BSCP</div>
  </div>
  <div style="text-align: center;">
    <img src="/assets/images/about/cbbh.png" alt="CBBH" style="width: 100px; height: auto;">
    <div style="font-weight: bold; font-size: 1.2em; margin-top: 10px;">CBBH</div>
  </div>
  <div style="text-align: center;">
    <img src="/assets/images/about/oswe.png" alt="OSWE" style="width: 100px; height: auto;">
    <div style="font-weight: bold; font-size: 1.2em; margin-top: 10px;">OSWE</div>
  </div>
  <div style="text-align: center;">
    <img src="/assets/images/about/ewptx.png" alt="EWPTX" style="width: 95px; height: auto;">
    <div style="font-weight: bold; font-size: 1.2em; margin-top: 10px;">EWPTX</div>
  </div>
</div>


